...
// Same dark palette as App.jsx (not exported from there, so redefined here)
const C = {
...
const styles = {
  ...
  kpiGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginBottom: 14 },
  ...
  kpiValue: { fontSize: 24, fontWeight: 700, color: C.text },
...
const CATEGORY_COLORS = {
  ...
  tires: "#ef4444", registration: "#22c55e", road_toll: "#f97316", other: "#8b949e",
};
...
export default function AnalyticsReportingView({ companyId, onToast }) {
  ...
  const [sortKey, setSortKey] = useState("totalApprovedCost");
  ...
  const plateFor = (vehicleId) => vehicles.find((v) => v.id === vehicleId)?.plateNumber ?? `#${vehicleId}`;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={styles.kpiGrid}>
        <div style={styles.kpi}>
          <div style={styles.kpiLabel}><DollarSign size={13} />Cumulative Spend</div>
          <div style={styles.kpiValue}>{money(data?.kpis?.cumulativeCompanySpend)}</div>
          <div style={styles.kpiSub}>Approved, non-fuel expenses in range</div>
        </div>
        <div style={styles.kpi}>
          <div style={styles.kpiLabel}><RouteIcon size={13} />Total Distance Logged</div>
          <div style={styles.kpiValue}>{new Intl.NumberFormat("en-ET").format(data?.kpis?.totalDistanceLogged ?? 0)} km</div>
          <div style={styles.kpiSub}>{vehicleId ? "1 vehicle" : `${vehicles.length} vehicles`}</div>
        </div>
        <div style={styles.kpi}>
          <div style={styles.kpiLabel}><Gauge size={13} />Average Fleet Efficiency</div>
          <div style={styles.kpiValue}>
            {data?.kpis?.averageFleetEfficiency == null ? "N/A" : `${money(data.kpis.averageFleetEfficiency)}/km`}
          </div>
          <div style={styles.kpiSub}>Cost per kilometer (CPK)</div>
        </div>
      </div>
      ...
      <div style={styles.panelGrid}>
        <div style={styles.card}>
          ...
          {!data?.expenseBreakdown?.length ? (
            <div style={styles.empty}>No approved expenses in this range.</div>
          ) : data.expenseBreakdown.map((row) => (
            <div key={row.category} style={styles.categoryRow}>
              <div style={styles.categoryTop}>
                <span style={styles.categoryName}>{row.category.replace("_", " ")}</span>
        ...
        <div style={styles.card}>
          ...
          {!data?.cpkTrend?.length ? (
            <div style={styles.empty}>No CPK data points in this range.</div>
          ) : (
            <div style={styles.trendRow}>
              {data.cpkTrend.map((point) => {
                const heightPct = point.costPerKilometer ? Math.max(4, (point.costPerKilometer / maxTrendValue) * 100) : 4;
                return (
                  <div key={point.period} style={styles.trendBarWrap}>
                    <span style={styles.trendValue}>{point.costPerKilometer == null ? "N/A" : point.costPerKilometer.toFixed(1)}</span>
                    <div style={{ ...styles.trendBar, height: `${heightPct}%` }} />
                    <span style={styles.trendLabel}>{new Date(point.period).toLocaleDateString("en-ET", { month: "short", day: "2-digit" })}</span>
                  </div>
                );
              })}
            </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  ...
                  <SortHeader field="totalDistanceCovered">Total Distance</SortHeader>
                  <SortHeader field="totalApprovedCost">Total Approved Cost</SortHeader>
                  <SortHeader field="costPerKilometer">Efficiency (CPK)</SortHeader>
                </tr>
              </thead>
              <tbody>
                {sortedComparison.map((row) => (
                  <tr key={row.vehicleId}>
                    ...
                    <td style={styles.td}>{new Intl.NumberFormat("en-ET").format(row.totalDistanceCovered)} km</td>
